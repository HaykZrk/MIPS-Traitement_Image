...
.data
    ...
    # Contient le nombre de bits utilises pour coder la couleur (non utilisé)
...
# Insertion du nom de l'image par l'utilisateur
...
# Fonction qui decale la lecture de 4 octets 
...
# pris en compte le seuillage et la valeur absolue.
sobelFilter:
    # Prologue
    subu $sp $sp, 16
    sw $ra, 0($sp)
...
startSobelFilterColumnLoop:
    beq $t3 $s1 startSobelFilterColumnLoopPass
        ...
        # Deuxieme partie de la matrice
            # Fonction decalage d'une ligne pour l'image originale
            li $t8 0 
startLine1FeedLoop:
...
GY:         # Pour GY   
            ...
            # Addition des matrices avec seuillage à 130
addMatrix:         
            add $t6 $t6 $t9
            

            bge $t6 255 byte255GXGY
                ble $t6 130 byteZeroGXGY
                    sb $t6 0($t0)
                    addi $t0 $t0 1
